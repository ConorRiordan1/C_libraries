...
static int create_socket6(int my_port) 
{
...
static int create_socket4(int my_port) {
...
static void init_socket(client_t *socket) {
 socket->last_message_time = 0;
 socket->socket_fd = 0;
 socket->session_id = 0;
 socket->client_account = NULL;
...
static void reset_socket(client_t *socket) {

 close(socket->socket_fd);


 if (NULL != socket->client_account) {
   helgrind_printf("client logged out\n",NULL);
   pthread_mutex_lock(&socket->client_account->mutex);
   socket->client_account->logged_in = 0;
   pthread_mutex_unlock(&socket->client_account->mutex);
 }
...
static void check_for_timeouts(client_t *clients, int num_clients) {
...
static void process_existing_client(int idx, size_t max_recv_size, linkedlist * p_maplist, linkedlist * p_accountlist)
{
  ...
  size_t valread = recv(client_sockets[idx].socket_fd, buffer, max_recv_size,0);
  ...
  if (0 == valread || valread > max_recv_size)
  {
    ...
    pthread_mutex_unlock(&client_sockets[idx].mutex);
    free(buffer);
    helgrind_printf("A client has left or been booted\n",NULL);
  ...
  else
  {
    ...
    process_data(&client_sockets[idx], buffer, valread, p_maplist,
                p_accountlist);
    free(buffer);
  }
...
static void handle_new_connection(int new_socket)
{
...
int server(linkedlist *p_maplist, linkedlist *p_accountlist, int port, int ipv)
{
  ...
  // init of basics, no connections, port is provided, new_socket used to represent client
  ...
  size_t max_recv_size = sizeof(login_command_t);

  if (sizeof(linkadd_command_t) > max_recv_size)
  {
    max_recv_size = sizeof(linkadd_command_t);
  }
  if (sizeof(user_del_command_t) > max_recv_size)
  {
    max_recv_size = sizeof(user_del_command_t);
  }
 ...
 fds[0].revents = 0;     /// occurances
  ...
  for (;;)
  {
   ...
   // conditional triggered on new connection only + enough space to add new connection
    if ((fds[0].revents & POLLIN) && (CONN_MAX > total_connections))
    {
      ...
      {
        helgrind_printf("New connection\n",NULL);
    ...
    for (int idx = 1; idx < CONN_MAX; idx++)
    {
     ...
     /// Triggered if an exisitng connection sends something
      ...
      process_existing_client(idx,max_recv_size,p_maplist,p_accountlist);
